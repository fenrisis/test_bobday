...
async def process_data():
    ...
    raw_data = await fetch_data(url_get, username, password)  # Убран дублирующийся вызов
    transformed_data = transform_data(raw_data)
    # Преобразование в dict для сериализации в JSON
    ...
    with open("output.json", "w") as f:
        # Сохраняем как JSON строку для корректной сериализации
...
def transform_data(raw_data):
    ...
    for employee_data in raw_data["employees"]:
        ...
        # Инициализация Employee из словаря
    ...
    # Возвращаем словарь объектов Department, каждый из которых преобразуется в dict
